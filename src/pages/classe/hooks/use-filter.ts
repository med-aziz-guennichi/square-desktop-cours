...
export function useClassFilters(classes: ClassData[] = []) {
  ...
  const filteredClasses = useMemo(() => {
    return classes?.filter((classe) => {
      // Search filter
      if (filters.search && !classe.name.toLowerCase().includes(filters.search.toLowerCase())) {
        return false
      }
      ...
      if (filters.gender) {
        const maleCount = classe.students?.filter((std) => std?.gender === "homme").length
        const femaleCount = classe.students?.filter((std) => std?.gender === "femme").length
        ...
        if (filters.gender === "balanced" && Math.abs(maleCount - femaleCount) > 3) {
          return false
        }
      }
  ...
  const updateFilter = (filterType: keyof ClassFilters, value: any) => {
    setFilters((prev) => ({
      ...prev,
      [filterType]: value,
    }))
