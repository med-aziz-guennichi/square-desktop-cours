...
export function CourseContent({ data }: { data: Chapters }) {
  ...
  const [activeVideo, setActiveVideo] = useState(data?.studyMaterials?.[0]?.fileName)
  ...
  if (!data?.studyMaterials?.length) {
    return <div className="flex items-center justify-center h-64 text-muted-foreground">No content available</div>
  }
    ...
    <div className="container mx-auto p-4">
      <div className="flex flex-col-reverse lg:flex-row gap-6 justify-between flex-1 order-1">
        ...
        <div className="flex-1 order-1">
          {data?.type === "Document" ? (
            <DocumentGallery enterprise={user?.enterprise || ""} documentData={data.studyMaterials[activeIndex]} />
          ) : data?.type === "Video" ? (
            ...
            <div className="flex items-center justify-center h-64 text-muted-foreground">Unsupported content type</div>
          )}
        </div>
              ...
              <div className="space-y-2">
                {data.studyMaterials.map((material, index) => (
                  <button
                    key={index}
                    onClick={() => setActiveIndex(index)}
                    className={`block w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-colors ${index === activeIndex ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                      }`}
                  >
