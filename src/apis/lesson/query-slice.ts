...
export async function getOneLesson(lessonId: string) {
  try {
    const response = await instance.get(
      `${API_ENDPOINT.ONELESSON}/${lessonId}`,
    );
    return response.data;
  } catch (error) {
    toast.error(
...
export async function getProgress(lessonId: string) {
  try {
    const response = await instance.get(
      `${API_ENDPOINT.PROGRESS}/${lessonId}`,
    );
    return response.data;
  } catch (error) {
    toast.error(
...
export async function getChapter(chapterId: string) {
  try {
    const response = await instance.get(
      `${API_ENDPOINT.CHAPTER}/${chapterId}`,
    );
    return response.data;
  } catch (error) {
    toast.error(
      'Une erreur s\'est produite lors de la récupération du chapitre.',
    )
